...
theorem exists_gcd_eq_one (a b : ℤ) :
  ...
  by_cases h : a = 0 ∧ b = 0
  · refine ⟨0, 1, 1, ?_⟩
    simp only [h.1, h.2, gcd_self, natAbs_zero, CharP.cast_eq_zero, mul_one, isUnit_one,
...
-- #find_home Int.exists_gcd_eq_one -- LegendreQF.Auxiliary, not particularly helpful
...
theorem exists_gcd_gcd_eq_one (a b c : ℤ) :
  ...
  · have hw₁ : 0 ≤ w := by
    ...
    refine
      ⟨g, a₁, w * b₂, w * c₂, by rwa [hg₁] at hg, hga, by rw [hgb, hgw, mul_assoc],
        by rw [hgc, hgw, mul_assoc], ?_⟩
    convert hg'
    simp only [gcd_mul_left, hg₁', mul_one, natCast_natAbs, abs_eq_self, hw₁]
...
/-- Two integers are not coprime if and only if there is a prime that divides both. -/
