...
theorem of_mul_right {a m n : R} (h : IsSquareMod a (m * n)) : IsSquareMod a n := by
  rw [mul_comm] at h
  exact h.of_mul_left
...
theorem mul {a b m : R} (ha : IsSquareMod a m) (hb : IsSquareMod b m) :
  ...
  obtain ⟨x, y, hb⟩ := hb
  exact
    ⟨u * x, u ^ 2 * y + x ^ 2 * v + m * v * y, by linear_combination b * ha + (u ^ 2 + m * v) * hb⟩
...
theorem of_mul_sq_of_coprime_left {a b m : R} (hs : IsSquareMod (a * b ^ 2) m)
    (hc : IsCoprime m b) : IsSquareMod a m := by
  rw [mul_comm] at hs
...
theorem sum_of_squares_of_isSquareMod_neg_one {b : ℤ} (hb : 0 ≤ b) (h : IsSquareMod (-1) b) :
    ∃ x y : ℤ, b = x ^ 2 + y ^ 2 := by
  lift b to ℕ using hb
  replace h : IsSquare (-1 : ZMod b)
  · convert (iff_isSquare' _ b).mp h
    push_cast
    rfl
